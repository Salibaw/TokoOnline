    ...
    <section class="section-checkout pt-4">
        <div class="container">
            <div class="text-center mb-4">
                <img src="{{asset('front-assets/images/download.jpg')}}" alt="Celebration Icon"
                    class="img-fluid celebration-icon">
            </div>
    ...
    document.getElementById('pay-button').addEventListener('click', function () {
        snap.pay('{{ $snapToken }}', {
            onSuccess: function (result) {
                ...
                console.log(result);
                window.location.href = '/my-orders?status=success';
            },
            onPending: function (result) {
                alert('Payment pending!');
                console.log(result);
                window.location.href = '/my-orders?status=pending';
            },
            onError: function (result) {
                alert('Payment failed!');
                console.log(result);
                window.location.href = '/my-orders?status=failed';
            },
        });
    });
</script>
